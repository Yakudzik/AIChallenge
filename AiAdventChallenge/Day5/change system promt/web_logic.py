...
def _format_payload(
    ...
    if json_mode == JSON_MODE_OFF:
        return answer
    payload = _build_payload(answer, provider, processing_time_ms, temperature)
...
def _handle_command(text: str, user_data: dict, chat_data: dict) -> list[str] | None:
    ...
    if command.startswith("/set_system_prompt"):
        ...
        if prompt_text:
            ...
            return ["Системный промпт обновлен."]
        user_data.pop(SYSTEM_PROMPT_KEY, None)
        return ["Системный промпт сброшен к значению по умолчанию."]
...
def process_text(text: str, user_data: dict, chat_data: dict) -> list[str]:
    ...
    try:
        ...
        if discussion_mode:
            ...
            discussion_memory = {label: content for _, label, content in answers}
            chat_data[DISCUSSION_MEMORY_KEY] = discussion_memory
            output = []
            for answer_provider, role, content in answers:
            ...
            referee_text = generate_referee_answer(
                discussion_memory,
        ...
        if clarify_state:
            ...
            summary = summarize_with_answers(
                clarify_state["original"],
